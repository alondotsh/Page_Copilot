...
扩展开源、本地优先，采用 Manifest V3，可通过内置提供商预设或自定义兼容接口接入你自己的模型 API Key。
...
- **自带模型 API**：可使用主流提供商预设，也可以手动填写 OpenAI-compatible 或 Anthropic-compatible 自定义接口
...
- **已内置提供商**：Claude、GLM、OpenAI、Gemini、Grok、DeepSeek、Qwen、Kimi、OpenRouter、SiliconFlow、MiniMax、MiMo，以及自定义兼容接口
- **提供商测试状态**：GLM 是当前主要测试过的提供商。其他内置预设基于兼容 API 格式接入，实际可用性可能受各提供商当前 API 行为影响，欢迎反馈和修复。
...
- 内置提供商预设用于降低配置成本；也可以手动配置自定义兼容接口
